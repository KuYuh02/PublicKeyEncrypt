...
keypair create_keys(const mpz_class& p, const mpz_class& q) {
	...
	mpz_class e = 65537; // Common choice for e (2^16 + 1)
...
mpz_class validate(const mpz_class& cypher, const mpz_class& signature, const keypair& pats_public_key) {
	mpz_class message = power_mod(cypher, private_key, public_key.first);
	mpz_class verified = power_mod(signature, pats_public_key.second, pats_public_key.first);
	uint16_t actual = sha16(message);

	if (verified == actual) {
		char response = (message % 2 == 0) ? 'e' : 'o';
		return power_mod(mpz_class(response), pats_public_key.second, pats_public_key.first);
	}

	return power_mod(mpz_class('f'), pats_public_key.second, pats_public_key.first);
}
